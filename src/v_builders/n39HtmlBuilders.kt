...
fun renderProductTable(): String {
    return html {
        table {
            ...
            val products = getProducts().mapIndexed { index, product ->
                tr {
                    td(color = getCellColor(row = index, column = 0)) {
                        text(product.description)
                    }
                    td(color = getCellColor(row = index, column = 1)) {
                        text(product.price)
                    }
                    td(color = getCellColor(row = index, column = 2)) {
